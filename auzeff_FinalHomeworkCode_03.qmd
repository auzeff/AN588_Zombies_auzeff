...
```{r}
# First, it would be convenient to have a dataset with only numeric columns.  For simplicity, I'll still use the given dataset for most steps.
...
# While I designed my code to print nicely formatted responses, it would also be good to have a dataframe containing just the numeric values of each calculation.  I can make that easily like so:
pop_means <- sapply(z_num, mean)
pop_sds <- sapply(z_num, pop_sd)
pop_stats <- data.frame(Mean = pop_means, SD = pop_sds)
pop_stats <- pop_stats[-1, ] # removing the id row
pop_stats
```
...
# A function for making the boxplots
bp_maker <- function(data, x, y, xlab, ylab) {
  bp <- ggplot(data = data, aes(x = x, y = y))
...
bp <- bp_maker(z, z$gender, z$height, "Gender", "Height (in)")
bp2 <- bp_maker(z, z$gender, z$weight, "Gender", "Weight (lb)")
bp3 <- bp_maker(z, z$gender, z$age, "Gender", "Age")
bp4 <- bp_maker(z, z$gender, z$zombies_killed, "Gender", "Number of zombies killed")
bp5 <- bp_maker(z, z$gender, z$years_of_education, "Gender", "Years of education")
...
```{r}
bp7 <- bp_maker(z, z$gender, z$weight, "Gender", "Weight (lb)")
bp7
```
...
whole_package <- function(x) {
  ...
  y <- gsub("_", " ", y)
  print(paste("Mean", y, "is", round(mean(x), 2)), quote = FALSE)
  print(paste("Standard deviation of", y, "is", round(sd(x), 3)), quote = FALSE) # I can use the base sd() function here, as it is a sample.
