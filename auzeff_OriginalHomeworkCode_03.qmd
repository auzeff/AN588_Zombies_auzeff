...
\* A quick note: I'm aware that I didn't use the correct theme. I much prefer using Quarto over RMarkdown, and I couldn't seem to get *'readthedown'* to work in it. If this is an issue, then I can port everything over to RMarkdown.
...
print(paste("Mean height is", mean(z$height)))
...
plots <- function(x) {
  ...
  qqnorm(x, main = paste("Normal QQ plot of", y))
  qqline(x, col = "cornflowerblue")
}
```
...
This looks to me more like a Poisson distribution, which would make sense as it is used for counts of independently occuring events. I can generate a probability mass function for a Poisson distribution to compare it to:
...
Again, clearly not a normal distribution. I'll unstandardize and try the Poisson distribution again:
...
As we can see, the sample deviations of the means are generally very similar to the standard errors calculate in question 5. The histograms and QQ plots also indicate that these distributions are roughly normal, including those that weren't normal at all in our initial sampling. This makes sense, as in this case we're not graphing the distributions of the variables themselves, but rather the means of our samples of these variables; as such, we would expect them to be normally distributed about the population mean.
