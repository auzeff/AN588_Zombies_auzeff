...
As we can see, the sample deviations of the means are generally very similar to the standard errors calculate in question 5. The histograms and QQ plots also indicate that these distributions are roughly normal, including those that weren't normal at all in our initial sampling. This makes sense, as in this case we're not graphing the distributions of the variables themselves, but rather the means of our samples of these variables; as such, we would expect them to be normally distributed about the population mean (the Central Limit Theorem).
